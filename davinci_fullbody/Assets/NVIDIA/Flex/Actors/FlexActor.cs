...
namespace NVIDIA.Flex
{
    ...
    public class FlexActor : MonoBehaviour
    {
        ...
        private List<int> FindParticleInRadius(Vector4[] allParticles, Vector3 pos, float radius)
        {
            ...
            Debug.Log("Number of grabbed particles: " + particlesUnderRadius.ToArray().Length);
            ...
            if (m_currentAsset.name == "Fat_FlexSoftAsset")
            {
                ...
                if ((oneTimePick) && !activeGrab )
                {
                    if (FindNextTo(allParticles, EE.transform.position)[1] < 0.02f) {
                        ...
                        particlesUnderRadius = FindParticleInRadius(allParticles, EE.transform.position, 0.05f);
